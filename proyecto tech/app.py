...
def index():
    ...
    renewable_data['Year'] = pd.to_numeric(renewable_data['Year'], errors='coerce')
    renewable_data = renewable_data.dropna(subset=['Year', 'Total Renewable Energy'])

    fig, ax = plt.subplots(figsize=(6, 5))

    ax.fill_between(renewable_data['Year'], renewable_data['Total Renewable Energy'], 
                    color='green', alpha=0.5, label='Energia Renovable')

    ax.set_title('Comparacion entre  colubna de energia Renovable y Convencional', fontsize=12)
